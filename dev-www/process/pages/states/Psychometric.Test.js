var irf = irf || {};
irf.pages.factory("PsychometricTestService", ["$q", "$state", "$rootScope", function($q, $state, $rootScope){
	return {
		start: function(participantId, applicationId) {
			irf.PsychometricTestServiceDeferred = $q.defer();
			$state.go("Page.PsychometricTest", {
				participantId: participantId,
				applicationId: applicationId
			});
			return irf.PsychometricTestServiceDeferred.promise;
		},
		deferred: function () {
			return irf.PsychometricTestServiceDeferred;
		},
		close: function () {
			irf.PsychometricTestServiceDeferred = null;
		}
	};
}]);
irf.pages.controller("PsychometricTestCtrl",
["$log", "$scope", "SessionStore", "$state", "$stateParams", "Psychometric", "$element", "$timeout", "Queries",
"PsychometricTestService", "PageHelper",
function($log, $scope, SessionStore, $state, $stateParams, Psychometric, $element, $timeout, Queries,
	PsychometricTestService, PageHelper){
	$log.info("PsychometricTest loaded");

	$scope.participantId = $stateParams.participantId;
	$scope.applicationId = $stateParams.applicationId;
	$scope.createdBy = SessionStore.getLoginname();

	// UI related starts
	$scope.participantCustomer = $scope.participantId;
	$scope.createdByUser = SessionStore.getUsername();

	Queries.getCustomerBasicDetails({ids:[$scope.participantId]}).then(function(resp){
		$scope.participantCustomer = resp.ids[$scope.participantId]['first_name'];
	});
	// UI related ends

	if (!(PsychometricTestService && _.isFunction(PsychometricTestService.deferred) && PsychometricTestService.deferred())) {
		$log.error("Psychometric Test Service Requested through invalid route");
		$state.go("Page.Engine", {"pageName": "psychometric.Queue", "pageId": null});
		return;
	}

	$scope.currentIndex = 0;
	$scope.indexText = function() {
		var i = $scope.currentIndex + 1;
		return i < 10 ? '0'+i:i;
	};

	$scope.LANGUAGES = [
		{
			"langCode":"en",
			"language":"English"
		}
	];
	Psychometric.getLanguages(null, function(resp){
		$scope.LANGUAGES = resp.body;
	});

	var STAGES = {
		"LANG_CHOICE": "LANG_CHOICE",
		"INSTRUCTIONS": "INSTRUCTIONS",
		"TEST": "TEST",
		"END": "END"
	};
	
	$scope.STAGES = STAGES;

	$scope.stage = STAGES.LANG_CHOICE;
	$scope.chosenLanguage = null;
	$scope.testPaused = false;
	var testCountdownId = null, breakCountdownId = null, testResumed = true;
	var startCountdown = function() {
		$scope.testStatus = 'Answering';
		var testDuration = $scope.test.testDuration;
		var totalTime = testDuration * 1000, startTime = new Date().getTime();

		testCountdownId = setInterval(function(){
			var durationElem = $element.find(".test-duration");
			if ($scope.testPaused) {
				if (testResumed) {
					testResumed = false;
					totalTime = totalTime + startTime - (new Date().getTime());
					durationElem.addClass('breathe_888_tomato');
				}
			} else {
				if (testResumed) {
					testResumed = false;
					startTime = new Date().getTime();
					durationElem.removeClass('breathe_888_tomato');
				}
				var remainingTime = totalTime + startTime - (new Date().getTime());
				if (remainingTime <= 0) {
					remainingTime = 0;
				}
				durationElem.html(moment.utc(remainingTime).format("HH:mm[<em>] ss[</em>]"));
				if (remainingTime == 0) {
					clearInterval(testCountdownId);
					$scope.moveStage(STAGES.END);
				}
			}
		}, 250);
	};

	var startBreakCountdown = function() {
		var breakDuration = $scope.test.intervalDuration;
		var breakTotalTime = breakDuration * 1000, breakStartTime = new Date().getTime();

		breakCountdownId = setInterval(function(){
			var durationElem = $element.find(".break-duration");
			var remainingTime = breakTotalTime + breakStartTime - (new Date().getTime());
			if (remainingTime <= 0) {
				remainingTime = 0;
			}
			durationElem.html(moment.utc(remainingTime).format("HH:mm[<em>] ss[</em>]"));
			if (remainingTime == 0) {
				$timeout(function() {
					$scope.pauseResumeTestCountdown(false);
				});
			}
		}, 250);
	};

	$scope.pauseResumeTestCountdown = function(pause, $event) {
		if (pause) {
			testResumed = true;
			$scope.testPaused = true;
			startBreakCountdown();
		} else if ($scope.testPaused) {
			$scope.testPaused = false;
			testResumed = true;
			clearInterval(breakCountdownId);
			$scope.allowedIntervals--;
		}
		$event && $event.preventDefault();
		return false;
	};

	$scope.next = function() {
		if ($scope.currentIndex == $scope.lastIndex) {
			$scope.moveStage(STAGES.END);
		} else {
			$scope.currentIndex++;
		}
	};

	$scope.skip = function() {
		$scope.test.questions[$scope.currentIndex].answerId = 0;
		if ($scope.test.questions[$scope.currentIndex].linkOrder === 1) {
			$scope.next();
			$scope.test.questions[$scope.currentIndex].answerId = 0;
		}
		$scope.next();
	};

	var prepareQuestionnaire = function() {
		$scope.error = null;
		$scope.testStatus = 'Preparing';
		$scope.lastIndex = -1;
		$log.info("Chosen language: " + $scope.chosenLanguage);
		Psychometric.getTest({
			"participantId": $scope.participantId,
			"applicationId": $scope.applicationId,
			"createdBy": $scope.createdBy,
			"langCode": $scope.chosenLanguage
		}).$promise.then(function(test) {
			$scope.test = test;
			$scope.allowedIntervals = $scope.test.noOfInterval;
			$scope.lastIndex = $scope.test.questions.length - 1;
			$scope.testStatus = 'Prepared';
		}, function(err) {
			$scope.testStatus = 'Failed';
			$scope.error = err.data;
		});
	};

	var submitAnswers = function() {
		$scope.error = null;
		$scope.testStatus = 'Submitting';
		clearInterval(testCountdownId);
		var testToSend = _.clone($scope.test);
		testToSend.questions = [];
		for (var i = 0; i < $scope.test.questions.length; i++) {
			testToSend.questions.push({
				id: $scope.test.questions[i].id,
				answerId: $scope.test.questions[i].answerId
			});
		};
		$log.info(testToSend);
		Psychometric.postTest(testToSend, function(resp){
			$log.info(resp);
			$scope.testStatus = 'Close';
			$scope.closeTest();
		}, function(errResp) {
			$log.error(errResp);
			$scope.testStatus = 'Submission failed';
			$scope.error = errResp.data;
		});
	};

	$scope.moveStage = function(toStage) {
		$scope.stage = toStage;
		switch ($scope.stage) {
			case STAGES.INSTRUCTIONS:
				prepareQuestionnaire();
				break;
			case STAGES.TEST:
				startCountdown();
				break;
			case STAGES.END:
				submitAnswers();
				break;
		}
	}

	$scope.closeTest = function() {
		if (PsychometricTestService.deferred()) {
			if ($scope.testStatus == 'Close') {
				PsychometricTestService.deferred().resolve({
					participantId: $scope.participantId,
					applicationId: $scope.applicationId
				});
				PsychometricTestService.close();
			} else {
				$log.info("Returning with Test Status: " + $scope.testStatus);
				PsychometricTestService.deferred().reject({
					participantId: $scope.participantId,
					applicationId: $scope.applicationId
				});
			}
		} else {
			$log.info("Test called through invalid route");
		}
	}

	$scope.$on('$stateChangeStart', function(event, toState, toParams, fromState, fromParams){ 
		if ($scope.testStatus == 'Answering') {
			event.preventDefault();
		}
	});


	$scope.INSTRUCTIONS_TEXT = {
		"en": {
			"title": "Instructions",
			"agree": "I Agree & Start Test",
			"instruction_1": "Current question by total questions",
			"instruction_2": "Press here for break",
			"instruction_3": "View countdown of test duration",
			"instruction_4": "Choose an option",
			"instruction_5": "Press next",
			"text": [
				"This questionnaire is to be completed in 20 minutes.",
				"We acknowledge that all persons are different and the answers are not to form a judgement of who is right or who is wrong.",
				"There are many situations described which are imaginary and hypothetical and you might have come across some or not faced some. There are no right and wrong answers. Please choose the answer which describes you best.",
				"You have to answer assuming what you think you would do naturally in such a situation. Please do not spend time analysing and debating. We request you to give top of the mind answers. It is highly important that you answer all the questions.",
				"By taking this test you accept that the outcome of the test may be used along with other tools to make a decision to sanction a loan",
				"Each question is followed by either 2 3 4 or 5 options. You should choose the one you feel best describes you."
			]
		},
		"hi": {
			"title": "सूचना",
			"agree": "मैं सहमत हूं & प्रारंभ टेस्ट",
			"text": [
				"यह एक समयबद्ध परीक्षा है। परीक्षा के लिए 20 मिनट का समय है।",
				"हम स्वीकार करते हैं कि सभी व्यक्ति अलग होते हैं और उत्तरों से यह निर्णय नहीं लिया जासकता है कि कौन सही है या कौन गलत है।",
				"यह प्रश्नावली ऋण देने के लिए आपको परखने की हमारी प्रक्रिया का एक हिस्सा है।",
				"अनेक परिस्थितियों का वर्णन किया गया है जो काल्पनिक हैं और आपकासंभवतः कुछ परिस्थितियों से सामनाहुआहो या नहीं हुआहो। यहां कोई सही या गलत उत्तर नहीं है। कृपया वह उत्तर चुनें जो आपकासर्वश्रेष्ठवर्णन करता है।",
				"आपको यहअनुमानलगातेहुए उत्तर देना है कि आप अपनी राय में ऐसीपरिस्थिति में सहजतः क्याकरेंगे। कृपया विश्लेषण और बहस करने में समय नहीं लगाएं। हमारा अनुरोध है कि आप वह उत्तर दें जो आपके मन में सबसेऊपर है। यह जरूरी है कि आप सभी प्रश्नों का उत्तर दें।",
				"यहपरीक्षा देकर आप स्वीकार करते हैं कि कर्ज देने का निर्णय पूर्णतया कंपनी के स्वविवेक पर निर्भर करता है और कंपनी प्रश्नावली, आपके द्वारा प्रस्तुतदस्तावेजों और किसीअन्य विधि, जो कंपनी उपयुक्त समझे, के आधार पर अपना निर्णय लेसकती है।"
			]
		},
		"mr": {
			"title": "सूचना",
			"agree": "मी सहमत आहे आणि चाचणी प्रारंभ",
			"text": [
				"ही प्रश्नावली 20 मिनिटांमध्ये पूर्ण करायची आहे.",
				"सर्व व्यक्ती भिन्न असतात याची आम्हाला जाण आहे व कोण बरोबर आहे व कोण चूक आहे हे ठर्वियासाठीही उत्तरे विचारली जात नाहीत.",
				"बÚयाच घटनांचे वर्णन केले असून या घटना काल्पनिक व गृहीतात्मक आहेत. त्यापैकी काही घटना आपण अनुभवल्या असतील, काही अनुभवल्या नसतील. कोणत्याही उत्तरास चूक किंवा बरोबर म्हणता येणार नाही. त्यामुळे, कृपया आपले सर्वात तंतोतंत वर्णन करणारे उत्तर निवडा.",
				"अशा परिस्थितीमध्ये आपण स्वाभाविकपणे काय कराल हे गृहीत धरून आपणास उत्तर द्यायचे आहे. कृपया विश्लेषण व वादविवादात वेळ घालवू नका. आमची आपणास विनंती आहे की, आपल्या डोक्यात सुरूवतीला जे येते ते उत्तर द्या. आपण सर्व प्रश्नांची उत्तरे देणे अत्यंत आवश्यक आहे.",
				"प्रत्येक प्रश्नानंतर 2 3 4 किंवा 5 पर्याय दिले आहेत. आपल्याला जो पर्याय आपले तंतोतंत वर्णन करतो असे वाटते तो पर्याय आपण निवडला पाहिजे",
				"तुम्ही हि परीक्षा देताना मान्य करता कि ह्या परीक्षे चा निकाल आणि वेगळे पारियाय वापरून लोन देण्याची मान्यता मिळेल"
			]
		},
		"kn": {
			"title": "ಸೂಚನೆಗಳು",
			"agree": "ನಾನು ಒಪ್ಪುತ್ತೇನೆ & ಪ್ರಾರಂಭಿಸಿ ಟೆಸ್ಟ್",
			"text": [
				"ಪ್ರಶ್ನಾವಳಿಯನ್ನು 20ನಿಮಿಷಗಳಲ್ಲಿ ಪೂರ್ತಿಗೊಳಿಸಬೇಕು.",
				"ಎಲ್ಲ ವ್ಯಕ್ತಿಗಳೂ ವಿಭಿನ್ನರಾಗಿರುತ್ತಾರೆ ಮತ್ತು ಉತ್ತರಗಳನ್ನು ಯಾರು ಸರಿ ಅಥವಾ ಯಾರು ತಪ್ಪು ಎಂಬ ನಿರ್ಧಾರಕ್ಕೆ ಬರಲು ಬಳಸಲಾಗದು ಎಂಬುದು ನಮಗೆ ತಿಳಿದಿದೆ.",
				"ಹಲವು ಕಲ್ಪಿತ ಮತ್ತು ಊಹಿಸಿದ ಸಂದರ್ಭಗಳನ್ನು ವಿವರಿಸಲಾಗಿದೆ, ಇವುಗಳಲ್ಲಿ ಕೆಲವು ನೀವು ಎದುರಿಸಿರಬಹುದು ಹಾಗೂ ಇನ್ನು ಕೆಲವು ನೀವು ಎದುರಿಸಿಲ್ಲದಿರಬಹುದು. ಇಲ್ಲಿ ಸರಿ ಅಥವಾ ತಪ್ಪು ಉತ್ತರ ಎಂಬುದಿಲ್ಲ. ದಯವಿಟ್ಟು ನಿಮ್ಮನ್ನು ಅತ್ಯುತ್ತಮವಾಗಿ ವಿವರಿಸುವಂತಹ ಉತ್ತರವನ್ನು ಆಯ್ಕೆಮಾಡಿ.",
				"ಅಂತಹ ಸಂದರ್ಭದಲ್ಲಿ ನಿಮ್ಮ ಸಾಮಾನ್ಯ ಪ್ರತಿಕ್ರಿಯೆ ಹೇಗಿರುತ್ತದೆ ಎಂದು ಊಹಿಸಿಕೊಂಡು ನೀವು ಉತ್ತರಿಸಬೇಕು. ವಿಶ್ಲೇಷಿಸುತ್ತಾ, ವಾದಮಾಡುತ್ತಾ ಸಮಯ ಕಳೆಯಬೇಡಿ. ನೀವು ನಿಮ್ಮ ಮನಸ್ಸಿನ ಉನ್ನತ ಸ್ತರದಿಂದ ಉತ್ತರಿಸಬೇಕೆಂದು ನಾವು ಪ್ರಾರ್ಥಿಸುತ್ತೇವೆ. ನೀವು ಎಲ್ಲ ಪ್ರಶ್ನೆಗಳನ್ನೂ ಉತ್ತರಿಸುವುದು ತುಂಬಾ ಮುಖ್ಯ.",
				"ಈ ಪರೀಕ್ಷೆಯನ್ನು ತೆಗೆದುಕೊಳ್ಳುವ ಮೂಲಕ, ಸಾಲವನ್ನು ಮಂಜೂರು ಮಾಡುವುದನ್ನು ನಿರ್ಧರಿಸಲು ಇತರ ಸಾಧನಗಳೊಂದಿಗೆ ಪರೀಕ್ಷೆಯ ಫಲಿತಾಂಶವನ್ನು ಬಳಸಿಕೊಳ್ಳಬಹುದೆಂದು ನೀವು ಒಪ್ಪುವಿರಿ.",
				"ಪ್ರತಿ ಪ್ರಶ್ನೆಯ ನಂತರವೂ 2 3 4 ಅಥವಾ 5 ಆಯ್ಕೆಗಳಿವೆ. ನಿಮ್ಮನ್ನು ಅತ್ಯಂತ ಉತ್ತಮವಾಗಿ ವಿವರಿಸುವಂತಹ ಉತ್ತರವನ್ನು ಆಯ್ಕೆ ಮಾಡಿ."
			]
		},
		"ta": {
			"title": "அறிவுறுத்தல்கள்",
			"agree": "நான் ஏற்கிறேன் & டெஸ்ட் தொடங்கவும்",
			"text": [
				"இவ்வினாத் தொகுப்பை 20 நிமிடங்களில் நிறைவு செய்யவேண்டும்.",
				"அனைத்து நபர்களும் ஒவ்வொரு விதத்தில் வேறுபட்டவர்கள் தான். யாருடைய பதில் சரி அல்லது தவறு என்று கண்டறிவதற்கான பதில்களாக இது இருப்பதில்லை என உறுதி செய்கிறோம்.",
				"இரண்டு பிரிவுகளிலும் கொடுக்கப்பட்டுள்ள பல சூழ்நிலைகள் யாவும் கற்பனையே மேலும் அனுமானத்தினால் உருவாக்கப்பட்டவை. இவைகளின் சிலவற்றை நீங்கள் உங்கள் வாழ்க்கையில் சந்தித்திருக்கக்கூடும். இதில் சரியான பதில்கள்அல்லது தவறான பதில்கள் என்று ஏதுமில்லை. ஆகவே, உங்களைப் பற்றிச் சிறப்பாக விவரிக்கும் பதிலைத்  தயவுசெய்து தேர்வு செய்யவும்.",
				"சில சூழ்நிலையில் உண்மையில் நீங்கள் என்ன செய்வீர்கள் என்பதை மனதில் கொண்டு சில கேள்விகளுக்குப் பதிலளிக்க வேண்டும். இக்கேள்விகளை ஆய்வு செய்வதற்கும் விவாதம் செய்வதற்கும் தயவு செய்து நேரத்தை செலவிட வேண்டாம். நீங்கள் உங்கள் மனதில் தோன்றக்கூடிய பதில்களை மட்டும் அளிக்குமாறு உங்களிடம் கேட்டுக்கொள்கிறோம். நீங்கள் அனைத்து கேள்விகளுக்கும் பதிலளிக்க வேண்டியது மிகவும் முக்கியமாகும்.",
				"இத்தேர்வை எடுப்பதன் மூலம், இதன் முடிவுகளை கடன் கொடுக்கும் முடிவை எடுப்பதற்கு மற்ற காரணிகளுடன் சேர்த்து உபயோகிக்க நீங்கள் ஒப்புக்கொள்கிறீர்கள்.",
				"ஒவ்வொரு கேள்விக்கும் 2, 3, 4 அல்லது 5 பதில்கள் இருக்கும். உங்களை எது சிறப்பாக விவரிக்கிறது என்று நீங்கள் நினைக்கும் ஒன்றை மட்டும் தேர்வு செய்யவும்."
			]
		},
		"te": {
			"title": "సూచనలు",
			"agree": "నేను అంగీకరిస్తున్నాను & ప్రారంభం టెస్ట్",
			"text": [
				"ఈ ప్రశావళిని 20 నిముషాలలో పూర్తి చెయ్యాలి.",
				"అందరు వ్యక్తులు వేర్వేరుగా ఉంటారని మాకు తెలుసు. ఎవరు సరియైన వారు, ఎవరు సరికాని వావు నిర్ణయించుటకు కాదు.",
				"రేడు విభాగాలలో కూడా ఊహించబడిన మరియు సిద్ధాంతపరమైనవి చాల పరిస్థితులు వర్ణించబడినాయి. మరియు మీకు కొన్ని ఎదురై ఉండవచ్చు. లేదా కొన్ని ఎదురవ్వక పోవచ్చు. సరియైన లేదా సరికాని జవాబులు లేవు. మిమ్మల్ని ఉత్తమంగా వర్ణించే దానిని దయచేసి ఎంపిక చేసుకోండి.",
				"అట్టి పరిస్థితిలో సహజంగా మీరు ఏమి చేస్తారో ఊహించాలి. వాదన విశ్లేషణతో సమయం వృధా చేయకండి. మీరు మానసిక దృష్టితో జవాబివ్వాలని మేము కోరుతున్నాము. అన్ని ప్రశ్నలకు మీరు జవాబు ఇవ్వటం అత్యంత ముఖ్యం.",
				"ఈ పరీక్ష చేయడం ద్వారా ఒక లోను మంజూరు చేయడానికి  నిర్ణయం తీసుకోవదానికి ఇతర సాధనాలతో సహా పరీక్ష ఫలితాన్ని ఉపయోగించవచ్చని మీరు అంగీకరిస్తారు.",
				"ప్రతి ప్రశ్నకు 2,3,4 లేదా 5 ఎంపికలు ఉన్నాయి. మిమ్మల్ని ఉత్తమంగా వర్ణిస్తున్నదని మీరు భావించే దానిని మీరు ఎంపిక చేసుకోవాలి."
			]
		},
		"instructionImage": "data:image/png;base64,"
	};

}]);
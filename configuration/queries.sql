...
ReadyToDisburseAccountDetails.list = SELECT l.id as loanId ,l.account_number as accountNumber, l.loan_amount as loanAmount FROM loan_accounts l LEFT JOIN jlg_account_details jlga ON (l.account_number = jlga.account_number) LEFT JOIN jlg_groups jlg ON (jlg.group_code = jlga.group_code) WHERE loan_disbursement_date IS NULL AND jlg.current_stage = 'Completed'AND l.urn_no = :urn_no UNION SELECT l.id as loanId,l.account_number, l.loan_amount FROM loan_accounts l WHERE loan_disbursement_date IS NULL AND l.current_stage = 'Completed'AND (l.urn_no = :urn_no or l.applicant = :urn_no)

define({
    ...
    pageType: "Engine",
    dependencies: ["$log", "$q", "SessionStore", "$state", "formHelper", "$stateParams", "LoanAccount", "LoanProcess", "PageHelper", "Groups", "GroupProcess", "Utils", "elementsUtils", '$filter', 'LoanProducts', 'irfNavigator'],
    $pageFn: function($log, $q, SessionStore, $state, formHelper, $stateParams, LoanAccount, LoanProcess, PageHelper, Groups, GroupProcess, Utils, elementsUtils, $filter, LoanProducts, irfNavigator) {
        ...
        return {
            ...
            initialize: function(model, form, formCtrl) {
                ...
                promise.then(function(resp) {
                            ...
                            var data = [];
                            if (!axisRepayment) {
                                model.repayments = _.cloneDeep(resp.summaries);
                                for (var i = 0; i < model.repayments.length; i++) {
                    ...
                    form: [{
                        ...
                        "items": [{
                            ...
                            "condition": "model.siteCode == 'KGFS'",
                            "title": "PRINT",
                            "onClick": "actions.printGroupReceipt(model, formCtrl, form, $event)"
                        }]
                    }],
                    schema: function() {
                    ...
                    actions: {
                        submit: function(model, formCtrl, formName) {
                            ...
                            if (window.confirm(msg + "Are you Sure?")) {
                                ...
                                LoanAccount.groupRepayment(reqData, function(resp, headers) {
                                    ...
                                    _.forOwn(model.repaymentResponse, function(repaymentResponse) {
                                        repaymentResponse.paymentResponse = _.find(resp.repayments, {
                                            'accountNumber': repaymentResponse.accountId
                                        })
                                    })
                        ...
                        printGroupReceipt: function(model, formCtrl, form, $event) {
                            ...
                            var getPrintHeader = function(opts) {
                                ...
                                pData.addLine(opts['entity_name'], {
                                    ...
                                    font: PrinterConstants.FONT_SMALL_NORMAL
                                }).addLine("", {}).addKeyValueLine("Group Code", opts['group_code'], {
                                    font: PrinterConstants.FONT_SMALL_NORMAL
                            ...
                            var getPrintFooter = function(opts) {
                                ...
                                pData.addStrRepeatingLine("-", {
                                    ...
                                    'center': true,
                                    font: PrinterConstants.FONT_SMALL_NORMAL
                                }).addLine("", {}).addLine("", {}).addLine("Signature not required as this is an", {
                                    'center': true,
                                    font: PrinterConstants.FONT_SMALL_NORMAL

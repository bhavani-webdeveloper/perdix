define({
    pageUID: "arohan.dashboard.loans.individual.screening.detail.LoanApplicationView",
    pageType: "Engine",
        ...
        var navigateToQueue = function(model) {
                    ...
                    if (model.currentStage == 'Screening')
                        $state.go('Page.Engine', {
                            pageName: 'arohan.dashboard.loans.individual.screening.ScreeningQueue',
                            pageId: null
                        });
                    if (model.currentStage == 'Dedupe')
                        $state.go('Page.Engine', {
                            pageName: 'arohan.dashboard.loans.individual.screening.DedupeQueue',
                            pageId: null
                        });
                    if (model.currentStage == 'ScreeningReview')
                        $state.go('Page.Engine', {
                            pageName: 'arohan.dashboard.loans.individual.screening.ScreeningReviewQueue',
                            pageId: null
                        });
                    if (model.currentStage == 'Application')
                        $state.go('Page.Engine', {
                            pageName: 'arohan.dashboard.loans.individual.screening.ApplicationQueue',
                            pageId: null
                        });
                    if (model.currentStage == 'ApplicationReview')
                        $state.go('Page.Engine', {
                            pageName: 'arohan.dashboard.loans.individual.screening.ApplicationReviewQueue',
                            pageId: null
                        });
                    if (model.currentStage == 'Scrutiny')
                        $state.go('Page.Engine', {
                            pageName: 'arohan.dashboard.loans.individual.screening.ScrutinyQueue',
                            pageId: null,
                        });
                    if (model.currentStage == 'FieldAppraisal')
                        $state.go('Page.Engine', {
                            pageName: 'arohan.dashboard.loans.individual.screening.FieldAppraisalQueue',
                            pageId: null
                        });
                    if (model.currentStage == 'FieldAppraisalReview')
                        $state.go('Page.Engine', {
                            pageName: 'arohan.dashboard.loans.individual.screening.FieldAppraisalReviewQueue',
                            pageId: null
                        });
                    if (model.currentStage == 'CreditCommitteeReview')
                        $state.go('Page.Engine', {
                            pageName: 'arohan.dashboard.loans.individual.screening.CreditCommitteeReviewQueue',
                            pageId: null
                        });
                    if (model.currentStage == 'CentralRiskReview')
                        $state.go('Page.Engine', {
                            pageName: 'arohan.dashboard.loans.individual.screening.CentralRiskReviewQueue',
                            pageId: null
                        });
                    if (model.currentStage == 'ZonalRiskReview')
                        $state.go('Page.Engine', {
                            pageName: 'arohan.dashboard.loans.individual.screening.ZonalRiskReviewQueue',
                            pageId: null
                        });
                    if (model.currentStage == 'Sanction')
                        $state.go('Page.Engine', {
                            pageName: 'arohan.dashboard.loans.individual.screening.LoanSanctionQueue',
                            pageId: null
                        });

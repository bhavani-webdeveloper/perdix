define({
    pageUID: "intellecash.dashboard.lead.LeadReject",
    pageType: "Engine",
                    ...
                    if (reqData.lead.id) {
                        ...
                        if (reqData.lead.leadStatus == "FollowUp" && model.lead.currentStage == "Inprocess") {
                            LeadHelper.followData(reqData).then(function(resp) {
                                $state.go('dashboardLoanOriginationDashboard', null);
                            });
                        } else {
                            LeadHelper.proceedData(reqData).then(function(resp) {
                                irfNavigator.go({
                                    state: "Page.Adhoc",
                                    pageName: "intellecash.dashboard.lead.LeadDashboard"
                                });
                        ...
                        LeadHelper.saveData(reqData).then(function(res) {
                            LeadHelper.proceedData(res).then(function(resp) {
                                irfNavigator.go({
                                    state: "Page.Adhoc",
                                    pageName: "intellecash.dashboard.loans.LoanOriginationDashboard"
                                });
                            }, function(err) {
                                Utils.removeNulls(res.lead, true);
                                model.lead = res.lead;

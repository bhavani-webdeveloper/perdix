irf.pageCollection.factory(irf.page("audit.detail.SnapAuditDetails"), ["$log", "PageHelper", "$q", "Utils", "Audit", "formHelper", "$state", "$stateParams", "irfNavigator", "SessionStore",
    function($log, PageHelper, $q, Utils, Audit, formHelper, $state, $stateParams, irfNavigator, SessionStore) {
        return {
                ...
                var init = function(response) {
                    ...
                    self.form = [{
                        ...
                        items: [{
                            type: "button",
                            title: "UPDATE",
                            onClick: "actions.submit(model, formCtrl, form, 'submit')"
                        }, {
                            type: "button",
                            title: "PUBLISH",
                            onClick: "actions.submit(model, formCtrl, form, 'publish')"
                        }]
                    }];
                };
            ...
            actions: {
                submit: function(model, form, formName, actionType) {
                    PageHelper.clearErrors();
                    PageHelper.showLoader();
                    var reqData = _.cloneDeep(model.snap_audit);
                    if (actionType == 'submit') {
                        ...
                        reqData.status = 'P';
                        Audit.online.updateSnapAudit(reqData).$promise.then(function(res) {
                            PageHelper.showProgress("auditId", "Snap Audit published Successfully.", 3000);
                            irfNavigator.goBack();
                        }, function(errRes) {
                            PageHelper.showErrors(errRes);
                        }).finally(function() {

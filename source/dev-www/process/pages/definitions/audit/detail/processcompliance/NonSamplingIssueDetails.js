irf.pageCollection.factory(irf.page("audit.detail.processcompliance.NonSamplingIssueDetails"),
["$log", "$state", "irfNavigator", "$stateParams", "Audit", "PageHelper", "$q", "elementsUtils", "formHelper", "$filter", "User",
                ...
                if (issue.options.type_of_issue_options[j].option_id == manual_issue.option_id) {
                    if (issue.options.type_of_issue_options[j].marks === 0) {
                        if (!manual_issue.assignee_det || !manual_issue.assignee_det.length || !manual_issue.assignee_det[0].assignee_id) {
                            PageHelper.setError({message: "Option: <strong>" + issue.options.type_of_issue_options[j].option_label + "</strong> has 0 marks, which requires responsible person to be given"});
                            return false;
                        }
                    }
                }
        ...
        initialize: function(model, form, formCtrl) {
            ...
            var renderForm = function() {
                ...
                boxItems.push({
                    ...
                    getListDisplayItem: function(i) { return [i.name] },
                    search: function(i, fm, m) {
                        var f = $f("filter")(typeOfIssuesTitleMap, { "group": m.manual_issue.sub_process_id }, true);
                        return $q.resolve({ "headers": {"x-total-count": f.length}, "body": f });
                    },
                    onSelect: function() {
                        renderForm();
                ...
                boxItems.push({
                    ...
                    "outputMap": { "login": "manual_issue.assignee_det[0].assignee_id" },
                    "searchHelper": formHelper,
                    ...
                    "outputMap": { "login": "manual_issue.assignee_det[1].assignee_id" },
                    "searchHelper": formHelper,
                    search: function(inputModel, form, model) {
                        return User.query({
                            'login': inputModel.login,

irf.pageCollection.factory(irf.page("audit.detail.processcompliance.SampleIssues"),
...
function($log, $state, irfNavigator, $stateParams, $http, Audit, PageHelper, $q, elementsUtils, formHelper, User) {
    var master = null;
    var validateResponsibility = function(sample) {
                    ...
                    if (issue.options.type_of_issue_options[j].option_id == id.option_id) {
                        if (issue.options.type_of_issue_options[j].marks === 0) {
                            if (!id.assignee_det || !id.assignee_det.length || !id.assignee_det[0].assignee_id) {
                                PageHelper.setError({message: "Issue: <strong>" + (Number(i)+1) + "</strong> option: <strong>" + issue.options.type_of_issue_options[j].option_label + "</strong> has 0 marks, which requires responsible person to be given"});
                                return false;
                            }
                        }
                    }
    ...
    return {
        ...
        initialize: function(model, form, formCtrl) {
            ...
            var processSampleIssues = function(response) {
                ...
                if (model.sample.issue_details && model.sample.issue_details.length) { // existing issues from sample

irf.pageCollection.factory(irf.page("shramsarathi.dashboard.loans.individual.collections.P2PUpdate"),
		...
		actions: {
			...
			submit: function(model, form, formName){
                ...
                LoanProcess.p2pUpdate(model.promise, function(response){
                    if(model._screen && model._screen == "BouncePromiseQueue")
                        $state.go('Page.Engine', {pageName: 'shramsarathi.dashboard.loans.individual.collections.BouncePromiseQueue', pageId: null});
                    else if(model._screen && model._screen == "BounceRecoveryQueue")
                        $state.go('Page.Engine', {pageName: 'shramsarathi.dashboard.loans.individual.collections.BounceRecoveryQueue', pageId: null});
                    else if(model._screen && model._screen == "BounceQueue")
                        $state.go('Page.Engine', {pageName: 'shramsarathi.dashboard.loans.individual.collections.BounceQueue', pageId: null});

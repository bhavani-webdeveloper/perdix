irf.pageCollection.factory(irf.page("shramsarathi.dashboard.loans.individual.booking.PendingForPartner"),
    ...
    function($log, SessionStore,$state,$stateParams,irfNavigator, SchemaResource,PageHelper,Enrollment,formHelper,IndividualLoan,Utils,$filter,$q,irfProgressMessage, Queries,LoanProducts, LoanBookingCommons, BundleManager){
        ...
        var navigateToQueue = function(model){
            if(model.currentStage=='LoanInitiation')
                $state.go('Page.Engine', {pageName: 'shramsarathi.dashboard.loans.individual.booking.InitiationQueue', pageId:null});
            if(model.currentStage=='PendingForPartner')
                $state.go('Page.Engine', {pageName: 'shramsarathi.dashboard.loans.individual.booking.PendingForPartnerQueue', pageId:null});
            if(model.currentStage=='IfmrDO')
                $state.go('Page.Engine', {pageName: 'shramsarathi.dashboard.loans.individual.booking.IFMRDOQueue', pageId:null});
        };
                ...
                if ($stateParams.pageId) {
                    ...
                    IndividualLoan.get({id: $stateParams.pageId}).$promise.then(function(resp){
                        if (resp.currentStage != 'LoanInitiation' && resp.currentStage != 'PendingForPartner' && resp.currentStage != 'IfmrDO') {
                            ...
                            $state.go('Page.Engine', {pageName: 'shramsarathi.dashboard.loans.individual.booking.PendingQueue', pageId: null});
                            return;
                        }
                 ...
                 viewLoan: function(model, formCtrl, form, $event) {
                    Utils.confirm("Save the data before proceed").then(function() {
                        ...
                        irfNavigator.go({
                            ...
                            state: "Page.Engine",
                            pageName: "shramsarathi.dashboard.loans.individual.booking.PendingForPartner",
                            pageId: model.loanAccount.id
                        });
                    });
                },
                            ...
                            function(){
                                ...
                                IndividualLoan.create(reqData)
                                    ...
                                    .then(function(res){
                                        ...
                                        $state.go("Page.Engine",{pageName:"shramsarathi.dashboard.loans.individual.booking.LoanInput", pageId: model.loanAccount.id}, {reload:true});
                                    }, function(httpRes){
                                        PageHelper.showErrors(httpRes);
                                    })
                                    .finally(function(httpRes){
                    ...
                    Utils.confirm("Save the data before proceed").then(function() {
                        ...
                        irfNavigator.go({
                            ...
                            pageId: model.loanAccount.id
                             ...
                             state: "Page.Engine",
                            pageName: "shramsarathi.dashboard.loans.individual.booking.IFMRDO",
                            pageId: model.loanAccount.id
                        });
                    });
                },

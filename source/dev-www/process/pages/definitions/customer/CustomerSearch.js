irf.pageCollection.factory(irf.page("CustomerSearch"), ["$log", "formHelper", "Enrollment","$q", "Queries", "$state", "SessionStore", "Utils", "PagesDefinition", "irfNavigator",
    function($log, formHelper, Enrollment,$q, Queries, $state, SessionStore, Utils, PagesDefinition, irfNavigator) {
                ...
                getResultsPromise: function(searchOptions, pageOpts) { /* Should return the Promise */
					...
					var promise = Enrollment.search({
						...
						for (i in response.body) {
							var res = response.body[i];
							var centres = formHelper.enum('centre').data;
                            var centreName = null;
                            for (var i = 0; i < centres.length; i++) {
                                var centre = centres[i];
                                if (centre.code == res.centreId) {
                                    centreName = centre.name;
                                    res.centreName=centreName
                                }
                            }
						}
						var result = {
